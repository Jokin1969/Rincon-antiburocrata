...
export async function generateDeclaracionExenta(data) {
  ...
  const lugarFinal = lugar?.trim() || '_____________________'
  ...
  // ── 1. Institutional header table ──────────────────────────────────────────
  // Left cell: MAPA text  |  Right cell: DGSPABA (top), SGASCF (bottom, with border)
  ...
  children.push(pp([
    ...
    tr('   ' + (aduana || '___________________________________')),
  ], { before: 40, after: 40 }))
  ...
  children.push(pp([
    ...
    tr(':'),
  ], { align: AlignmentType.JUSTIFIED, before: 80, after: 80 }))

  // ── 6. Six underlined bullet points ─────────────────────────────────────────
  const bullets = [
  ...
  bullets.forEach(b => children.push(underlineBullet(b)))

  children.push(emptyLine(120))

  // ── 7. Second legal paragraph ───────────────────────────────────────────────
  children.push(pp([
  ...
  children.push(pp([tr('Nombre, apellidos y cargo', { bold: true })], { align: AlignmentType.CENTER, before: 40, after: 0 }))
  if (firmante) children.push(pp([tr(firmante)], { align: AlignmentType.CENTER, before: 0, after: 0 }))
